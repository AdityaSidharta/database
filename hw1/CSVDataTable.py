...
class CSVDataTable(BaseDataTable):
    ...
    def is_legal(self, rows):
        values = []
        for row in rows:
            value = ''.join(row[key_field] for key_field in self.get_key_column())
            values.append(value)
    ...
    def update_by_key(self, key_fields, new_values):
        ...
        for r in copy.deepcopy(self._rows):
            if self.matches_template(r, template):
                ...
                proposed_rows.append(r)
        if self.is_legal(proposed_rows):
    ...
    def insert(self, new_record):
        ...
        proposed_rows = copy.deepcopy(self._rows)
        proposed_rows.append(new_record)
        if self.is_legal(proposed_rows):
            self._rows = proposed_rows
        else:
            raise ValueError("new_record have primary key values equal to the ones in the current table")
