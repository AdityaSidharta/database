...
class CSVTable:
    ...
    def __get_access_path__(self, template):
        """
        ...
        """
        # Do some initial error checking to make sure there are indexes for the table and there are templates passed
        ...
        if template is None:
            return selected_index, selected_count
        elif not len (self.__indexes__):
            return selected_index, selected_count
        else:
            for index_name, index_dict in self.__indexes__.items():
                for index_key, rows in index_dict.items():
                    if all([template_values in index_key for template_keys, template_values in template.items()]):
                        if (selected_count is None) or (len(rows) < selected_count):
                            selected_index = (index_name, index_key)
    ...
    def __find_by_template_scan__(self, t, fields=None):
        ...
        for r in reversed(self.__get_row_list__()):
    ...
    def __find_by_template_index__(self, t, idx_name, fields=None):
        ...
        """
        result = []
        index_name, index_key = idx_name

        rows = self.__indexes__[index_name][index_key]
    ...
    def __smart_join__(self, right_r, on_fields, where_template=None, project_fields=None):
        ...
        """
        left_r = self
        left_rows = left_r.__find_by_template__(where_template)
        right_rows = right_r.__find_by_template__(where_template)
        result_rows = []
