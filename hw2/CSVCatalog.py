...
class TableDefinition:
    ...
    def load_columns(self):
        ...
        results = run_q(self.cnx, q, args, fetch = True)
        if len(results):
            ...
            for result in results:
                column_definition = ColumnDefinition(
                    ...
                    not_null = bool(result['not_null'])
                )
                if column_definition not in self.columns:
                    self.columns.append(column_definition)
        else:
            pass
        ...
        if len(results):
            ...
            for result in results:
                ...
                if index_name not in indices:
                    indices[index_name] = {}
                    indices[index_name]['index_type'] = index_type
                    indices[index_name]['column_name'] = []
                    indices[index_name]['column_order'] = []
                assert indices[index_name]['index_type'] == index_type
                indices[index_name]['column_name'].append(column_name)
                indices[index_name]['column_order'].append(index_order)
            for index_name, index_dict in indices.items():
                indexdefinition = IndexDefinition(
                    index_name = index_name,
                    index_type = index_dict['index_type'],
                    column_names = [index_dict['column_name'] 
                    for _, index_dict['column_name'] in sorted(zip(index_dict['column_order'] ,
                    index_dict['column_name'] ))]
                )
                if indexdefinition not in self.indexes:
    ...
    def load_core_definition(self):
        ...
        args = (self.table_name)
        result = run_q(self.cnx, q, args, fetch = True)
        if len(result):
            assert len(result) == 1
    ...
    def get_column(self, cn):
        ...
        for col in self.columns:
            if col.column_name == cn.column_name:
                return col

        print("Column '" + cn.column_name + "' not found")
        return
    ...
    def drop_column_definition(self, cn):
        ...
        if column_to_drop is not None:
            ...
            print("Column '" + cn.column_name + "' has been dropped!")
    ...
    def drop_col_in_sql(self, cn):
        ...
        result = run_q(self.cnx, q, (self.table_name, cn.column_name), fetch=True)
    ...
    def get_index(self, ind_name):
        ...
        if results:
            assert len(results) == 1
            for result in results:
                individual_type = result['type']
                column_name = result['column_name']
                index_order = result['index_order']

                if index_type is None:
                    index_type = individual_type
                else:
                    assert index_type == individual_type
                
                colnames.append(column_name)
                colorders.append(index_order)
            
            indexdefinition = IndexDefinition(
                index_name = ind_name,
                index_type = index_type,
                column_names = [colnames 
                for _, colnames in sorted(zip(colorders , colnames ))]
            )
    ...
    def drop_index(self, index_name):
        ...
        if index is not None:
            ...
            idx_to_remove = []
            for idx in range(len(self.indexes)):
                if (self.indexes[idx].index_name == index.index_name) & \
                (self.indexes[idx].index_type == index.index_type) & \
                (self.indexes[idx].column_names == index.column_names):
                    idx.to_remove.append(idx)
            if len(idx_to_remove):
                assert len(idx_to_remove) == 1
                self.indexes.pop(idx_to_remove[0])
